...
class DataStore:
    ...
    def _save_applied(self, names: list[str]) -> None:
        ...
        try:
            ...
            logger.error("applied_migrations_save_failed", path=str(self.applied_file), exc_info=e)
    ...
    def register_migration(
        self, version: str, name: str, description: str, fn: Callable[["DataStore"], bool]
    ) -> None:
    ...
    def get_pending_migrations(self) -> list[Migration]:
        ...
        pending = []
        for ver_str, migration in sorted(self._migrations.items(), key=lambda x: DataVersion(x[0])):
    ...
    def migrate(self) -> bool:
        ...
        if not pending:
            logger.info("no_pending_migrations", version=str(self.get_version()))
            return True

        logger.info("starting_migrations", count=len(pending), from_version=str(self.get_version()))
        ...
        for migration in pending:
            ...
            if not success:
                logger.error(
                    "migration_returned_false", version=str(migration.version), name=migration.name
                )
                return False
            ...
            logger.info("migration_success", version=str(migration.version), name=migration.name)

        logger.info("all_migrations_complete", final_version=str(self.get_version()))
        return True
